        ...
        StackLayout {
            ...
            Item {
                ...
                GridLayout {
                    ...
                    C.ComboBox {
                        readonly property var windowTypeValues: [0, 3, 4, 5]
                        Layout.preferredWidth: 400
                        model: [qsTr("Selected Resolution"), qsTr("Fullscreen"), qsTr("Zoom [adjust zoom using slider in stream menu]"), qsTr("Stretch")]
                        currentIndex: Math.max(0, windowTypeValues.indexOf(Chiaki.settings.windowType))
                        onActivated: (index) => Chiaki.settings.windowType = windowTypeValues[index]
                    }
                    ...
                    C.ComboBox {
                        readonly property var videoPresetValues: [0, 1, 2, 5]
                        Layout.preferredWidth: 400
                        model: [qsTr("Fast"), qsTr("Default"), qsTr("High Quality"), qsTr("Custom")]
                        currentIndex: {
                            const index = videoPresetValues.indexOf(Chiaki.settings.videoPreset)
                            return index >= 0 ? index : 2
                        }
                        onActivated: (index) => {
                            Chiaki.settings.videoPreset = videoPresetValues[index];
                            switch (index) {
